...
void MergeSort::merge( int left, int middle1, int middle2, int right ) 
{
   ...
   vector<Word> combo;// Results vector
   string w = "hello";
   string f = "sample";
   combo.reserve(size);
  for ( size_t y = 0; y < MatchedResults.size(); y++)
   {
	   combo.push_back(MatchedResults[y]);
   } 
 ...
 //  cout << endl;                         
   ...
   while ( leftIndex <= middle1 && rightIndex <= right )
   {
      ...
      // and move to next space in vector
	   if ( MatchedResults[ leftIndex ].getScore() <= MatchedResults[ rightIndex ].getScore() )
	   {
         //combined[ combinedIndex++ ] = MatchedResults[ leftIndex++ ].getScore(); 
		 Word theWord = MatchedResults[leftIndex++];
	     combo[ combinedIndex++ ] = theWord;
	   }
      else
	  {
		  //combined[ combinedIndex++ ] = MatchedResults[ rightIndex++ ].getScore();
		  Word theWord = MatchedResults[rightIndex++];
		  combo[ combinedIndex++ ] = theWord;
	  }
   } // end while
...
void MergeSort::displaySubVector( int low, int high ) const
{
 ...
 //  for ( int i = 0; i < low; ++i )
  ...
  // for ( int i = high; i >= low; --i )
     //cout << MatchedResults[i].getWord() << " - score: " << MatchedResults[i].getScore() << " at position " << MatchedResults[i].getPosition() << endl;
